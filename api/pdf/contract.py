...
def create_contract_pdf(employee: EmployeeRead, petition: PetitionRead) -> BytesIO:
    buffer = BytesIO()
    with Pdf.open(
        "".join([consts.PDF_TEMPLATE_DIR, consts.CONTRACT_PDF_TEMPLATE])
    ) as pdf:
        ...
        form[consts.BIRTHDAY_FIELD].value = (
            f"{employee.date_of_birth.strftime('%d.%m.%Y')}"
        )
        form[consts.ADDRESS_FIELD].value = f"{employee.address},{employee.postal_code}"
        form[consts.CONTRACT_START_FIELD].value = (
            f"{petition.start_date.strftime('%d.%m.%Y')}"
        )
        form[consts.CONTRACT_END_FIELD].value = (
            f"{petition.end_date.strftime('%d.%m.%Y')}"
        )
